...
const rp = abortableFetch(require('request-promise-native'))
const retry = abortableFetch(require('async-retry'))
...
/**
 * Check if valid Gateway URI
 *
 * @param {string} gatewayURI - The value to check
 * @returns {bool} true if value is a valid Gateway URI, otherwise false
 */
export function isValidGatewayURI(gatewayURI) {
  if (!isString(gatewayURI)) return false

  try {
    let isValidURI = isURL(gatewayURI, {
      protocols: ['http', 'https'],
      require_protocol: true,
      host_blacklist: ['0.0.0.0']
    })

    let parsedURI = parse(gatewayURI).hostname
...
export async function getGatewayListAsync(coreIPs) {
  ...
  let gatewayIPs = []
  let peersReceived = false
  while (!peersReceived && coreIPs.length > 0) {
...
/**
 * Test an array of Gateway uris to see if they are responding to requests.
 ...
 *
 * @param {String[]} gatewayUris - array of Gateway URIs
 ...
 */
export function testGatewayEndpoints(gatewayUris, failures = [], timeout = 150) {
  testArrayArg(gatewayUris)
  return Promise.all(
    gatewayUris.map(async gatewayUri => {
      try {
        ...
        await fetch(gatewayUri, { timeout, method: 'GET', signal })

        clearTimeout(timeoutId)
        return gatewayUri
      } catch (e) {
        failures.push(gatewayUri)
      }
    })
  )
}
