...
import { isHex, testArrayArg, isValidUUID } from './helpers'
...
/**
 * Map the JSON API response from submitting a hash to a Gateway to a
 * more accessible form that can also be used as the input arg to
 * getProofs function.
 *
 * @param {Array} respArray - An Array of responses, one for each Gateway submitted to
 * @returns {Array<{uri: String, hash: String, proofId: String}>} An Array of proofHandles
 */
export function mapSubmitHashesRespToProofHandles(respArray) {
  ...
  if (respArray[0] && respArray[0].hashes) {
    forEach(respArray[0].hashes, hash => {
      if (hash.hasOwnProperty('proof_id')) {
        if (isValidUUID(hash.proof_id)) {
          groupIdList.push(uuidv1())
          return
        }
      }
      groupIdList.push(UlidMonotonic.generate().toCanonical())
    })
  }
...
export function normalizeProofs(proofs) {
  ...
  forEach(proofs, proof => {
    if (isObject(proof) && has(proof, 'proof') && isString(proof.proof)) {
      ...
      console.error(`no proof for proofId ${proof.proofId}`)
    } else {
