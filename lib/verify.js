...
export default async function verifyProofs(proofs, uri) {
  ...
  let gatewayUri
  if (isEmpty(uri)) {
    gatewayUri = 'http://18.224.185.143'
  } else {
    if (!isString(uri)) throw new Error('uri arg must be a String')
    if (!isValidURI(uri)) throw new Error(`uri arg contains invalid Gateway URI : ${uri}`)
    gatewayUri = uri
  }
  ...
  let getResponses = fetchResults
    ...
    .filter(result => {
      if (result.error) {
        console.log(`Client error verifying proof on ${result.uri} : ${result.error} : Skipping`)
        return false
      }
      return true
    })
