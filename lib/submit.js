...
import { getCorePeerListAsync, getCores, getGatewayListAsync, isValidGatewayURI } from './utils/network'
...
export async function submitHashes(hashes, uris) {
  ...
  if (isEmpty(uris)) {
    try {
      ...
      gatewayUris = map(gateways, gatewayIP => {
        return 'http://' + gatewayIP
      })
    } catch (error) {
      console.log(`getting GatewayIps from network failed, falling back to defaults: ${error}`)
      gatewayUris = ['http://3.17.155.208', 'http://18.191.50.129', 'http://18.224.185.143']
    }
  } else {
