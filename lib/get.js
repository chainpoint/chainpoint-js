...
import { isValidUUID, fetchEndpoints, testArrayArg } from './utils/helpers'
...
async function getProofs(proofHandles) {
  ...
  let badHandleURIs = reject(proofHandles, function(u) {
    return isValidGatewayURI(u.uri)
  })
  ...
  try {
    ...
    let getResponses = fetchResults
      ...
      .filter(result => {
        if (result.error) {
          console.log(`Client error in get response from ${result.uri} : ${result.error} : Skipping`)
          return false
        }
        return true
      })
